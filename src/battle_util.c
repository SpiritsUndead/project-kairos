...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_INTIMIDATE:
            ...
            break;
        case ABILITY_HUMILIATE:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                gBattlerAttacker = battler;
                SET_STATCHANGER(STAT_SPATK, 1, TRUE);
                BattleScriptPushCursorAndCallback(BattleScript_HumiliateActivates);
                effect++;
            }
            break;
...
const struct TypePower gNaturalGiftTable[] =
{
    ...
    [ITEM_TO_BERRY(ITEM_LICAR_BERRY)] = {TYPE_PSYCHIC, 100},
    [ITEM_TO_BERRY(ITEM_TARNEC_BERRY)] = {TYPE_NORMAL, 100},
};
...
static inline u32 CalcAttackStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    case ABILITY_WHITEOUT:
        if (moveType == TYPE_ICE && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP / 3))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_ENCHANTMENT:
        if (moveType == TYPE_FAIRY && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP / 3))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_PLUS:
